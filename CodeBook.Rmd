...
```
```{r}
subsetmerge <- rbind(activity[], X_train)
View(subsetmerge)
```
...
```{r}
colnames(meanandstandardmeasure)
```


```{r}
# Extraer nombres del subset
nombres <- gsub("\\()", "",colnames(meanandstandardmeasure),) 
nombres2 <- gsub("-", "",nombres,)
nombres3 <- gsub("Acc", "Acceleration", nombres2)
